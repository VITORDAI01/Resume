...
function selectSourcesById(sourceIds) {
  return sourceIds
    .map((id) => index.chunks.find((chunk) => chunk.id === id))
    .filter(Boolean)
...
const server = createServer(async (req, res) => {
  ...
  try {
    ...
    if (curatedRoute) {
      const sources = selectSourcesById(curatedRoute.sourceIds);
      res.writeHead(200, {
        ...corsHeaders(origin),
        "Content-Type": "text/event-stream; charset=utf-8",
        "Cache-Control": "no-cache, no-transform",
      ...
      sendEvent(res, "sources", sources.map(({ id, title, section, sourceType, url: sourceUrl, text, score }) => ({
        id,
        title,
        section,
        sourceType,
      ...
      sendEvent(res, "token", { token: curatedRoute.reply });
      sendEvent(res, "done", { ok: true });
      res.end();
      return;
    }
    ...
    const sources = previousSourceIds.length > 0
      ? selectSourcesById(previousSourceIds)
      : retrieve(await embed(retrievalQuery), question);
